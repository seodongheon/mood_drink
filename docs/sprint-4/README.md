...
- [x] 네트워크 차단 또는 서버 500 에러 시 친절한 안내 멘트가 뜨고 버튼이 재활성화된다.
- [x] 3초 이상 소요 시 Fallback 결과가 노출되어 무한 로딩이 발생하지 않는다.
- [x] 추천 완료 후 '다시 추천받기' 버튼을 통해 연속적인 재추천이 가능하다.
